		 ...
		 <div class="content-asp-reg">
		  ...
		  <div class="col-md-4 col-sm-4 box-left-shadow">
			...
			<ul class="nav nav-tabs login-contain">
				<li class="<?php if(($tabs=='aspirant')|| ($tabs=='')) { echo "active";}?>"><a data-toggle="tab" href="#regisone" id="aspregimage" >Aspirant</a></li>
				<li class="<?php if(($tabs=='employer')) { echo "active";}?>"><a data-toggle="tab" href="#registwo" id="empregimage">Employer</a></li>
			</ul>
			...
			<div class="tab-content">
				...
				<form class="form popupmodal formregasp" autocomplete="off" >
					...
					<input type="text" class=" form-input form-control"  onKeyPress="return isString(event)"  maxlength="15" id="aspfirst" name="aspfirst" onkeyup="return aspregistration();">
					</div>
					<div class="col-md-6 form-group">
					<label class="form-label heading-title" for="last">Last name</label>
					<input type="text" class="form-input form-control"  onKeyPress="return isString(event)" maxlength="15" id="asplast" name="asplast"  onkeyup="return aspregistration();">
					</div>
                    </div>
				<div class="form-group position-relative">
                <label class="form-label heading-title" for="mobile">Mobile number</label>
                <input type="tel" class="form-input form-control" maxlength="15" onKeyPress="return isNumeric(event)"  name="aspmobile"  onkeyup="return aspregistration();">
				...
				<div class="form-group text-center otp-input allotp" style="display:none;">
					...
					<input type="tel" onKeyPress="return isNumeric(event)" maxlength="1" name="aspotpone" id="aspotpone"  onkeyup="return aspregistration();">
					<input type="tel" onKeyPress="return isNumeric(event)" maxlength="1" name="aspotptwo" id="aspotptwo"  onkeyup="return aspregistration();">
					<input type="tel" onKeyPress="return isNumeric(event)" maxlength="1" name="aspotpthree" id="aspotpthree"  onkeyup="return aspregistration();">
					<input type="tel" onKeyPress="return isNumeric(event)" maxlength="1" name="aspotpfour" id="aspotpfour"  onkeyup="return aspregistration();">
                </div>
				<div id="timer" class="settimer"></div>
				<div class="form-group">
                <label class="form-label heading-title" for="email">Email address</label>
                <input type="email" class="form-input form-control"  id="aspemail"  name="aspemail" onchange="return verifyemailtwo(this.value)">
                </div>
				<span id="asperror"></span>
				<button type="button" name="submit" value="submit" id="buttonregasp" class="btn btn-primary d-inline-block d-sm-block btn-modal" disabled onclick="return registeredasp();">Continue</button>
				...
				<p>Already Registered? <a href="<?php echo base_url().'login'?>" > Login here </a>.</p>
                </div>
				</div>
				...
				<form class="form popupmodal formregemp" autocomplete="off">
					...
					<div class="col-md-6 form-group">
					<label class="form-label heading-title" for="first">First name</label>
					<input type="text" class=" form-input form-control"  onKeyPress="return isString(event)"  maxlength="15" id="empfirst" name="empfirst" onkeyup="return regemp();">
					</div>
					<div class="col-md-6 form-group">
					<label class="form-label heading-title" for="last">Last name</label>
					<input type="text" class="form-input form-control"  onKeyPress="return isString(event)" maxlength="15" id="emplast" name="emplast" onkeyup="return regemp();">
					</div>
                    </div>
					<div class="form-group position-relative">
					<label class="form-label heading-title" for="mobile">Mobile number</label>
				...
				<button type="button" name="button" id="buttonregemp" class="btn btn-primary d-inline-block d-sm-block btn-modal" onclick="return registeredemp();" disabled>Continue</button>
...
<script>
	...
	if(( empfirst.trim()=='')||( emplast.trim()=='')||( empmobile.trim()=='')||( empmobile.length<='9')||( empemail.trim()=='')||(!testEmail.test(empemail))|| (inputotp=='')||(otpuotp=='')||(inputotp!=otpuotp)){
		$('#buttonregemp').attr('disabled',true);
...
function aspregistration(){
	...
	var testEmail = /^[A-Z0-9._%+-]+@([A-Z0-9-]+\.)+[A-Z]{2,4}$/i;
	if((aspfirst.trim()=='')||(asplast.trim()=='')||(aspmobile.trim()=='')||(aspmobile.length<='9')||(aspemail.trim()=='')||(!testEmail.test(aspemail)) || (inputotpp=='')||(inputotpp!=otpuotpp)){
  ...
  function attachSignin(element) {
    auth2.attachClickHandler(element, {},
        function(googleUser) {
			  ...
			  window.location = '<?php echo base_url();?>aspmobile?aspfirstname=' + btoa(getallname[0])+'&asplastname='+btoa(getallname[1])+'&aspemail='+btoa(aspemail);
    ...
    auth2.attachClickHandler(element, {},
        function(googleUser) {
			var aspname=googleUser.getBasicProfile().getName();
			  ...
			  window.location = '<?php echo base_url();?>empmobile?aspfirstname=' + btoa(getallname[0])+'&asplastname='+btoa(getallname[1])+'&aspemail='+btoa(aspemail);
...
function registeredasp(){
	...
	window.location = '<?php echo base_url();?>aspmobile?aspfirstname=' + btoa(aspname)+'&asplastname='+btoa(asplast)+'&aspemail='+btoa(aspemail)+'&aspmobile='+btoa(aspmobile);
}
...
function registeredemp(){
	...
	window.location = '<?php echo base_url();?>empmobile?aspfirstname=' + btoa(aspname)+'&asplastname='+btoa(asplast)+'&aspemail='+btoa(aspemail)+'&aspmobile='+btoa(aspmobile);
}
</script>
